...
### Flujo actual
- Tema visual crema (`#faf7ed`) con acento teal `#0f6b68` en todos los botones; header ocultable al scroll (evento `realdump:scroll-header`); barra inferior flotante `BottomNav.tsx`.
...
- **Sesión UI — tema crema + acento teal #0f6b68**: tema crema `--color-app-bg: #faf7ed` en `globals.css` (`@theme inline`) y `layout.tsx` (`bg-app-bg`, `text-zinc-900`). **Header ocultable al scroll** (`Header.tsx`: `SCROLL_REVEAL_TOP=60`, `SCROLL_THRESHOLD=10`; escucha scroll + evento `realdump:scroll-header` que dispara `VideoFeed` con `{deltaY, scrollY}`; setea `--header-offset` en `<html>` → `/search` usa `top-[var(--header-offset,3.5rem)]`). **Barra inferior flotante** `BottomNav.tsx` (pill `rounded-[25px] bg-[#F2F2F2]`). Fondos de video **siguen `bg-black`** (el crema se revirtió en `a6c0e69`). Redondeo `rounded-[15px]` en superficies de video (VideoFeed, ProfileVideoCard, ProfileVideoOverlay, CustomVideoPlayer, MuxVideoPlayer, FeedSkeleton). `/notificaciones` en cards claras `bg-[#F2F2F2]` sin borde, letra reducida. **Acento `#0f6b68` (hover `#0b5451`) en TODOS los botones**: sólidos (login/register/reactivate/editar/profile/edit/profile/filters/upload/verificacion/CameraCapture/HamburgerMenu/CommentsDrawer/ReportModal/AuthButton/notificaciones/admin/reports), contorno "Seguir"/estado "Siguiendo" (ProfileRow, user/[id], ProfileVideoOverlay), estados seleccionados (cards de registro, documento de verificación, razones/modo perfil del ReportModal), badge de la campana (`Header.tsx`). Se dejaron azules a propósito (no son botones): links de texto, carets/focos de inputs, tabs activos `border-b-2`, badges de estado, dots/progress/bell icon, checkmarks, hashtags. **Commits pusheados**: `ce684f2` tema crema + header ocultable; `1aa25ff`/`345a856`/`7ae6b5f`/`f46fa1f` re-proteger; `ea33e2e`+`a6c0e69` fondos video ida y vuelta; `d6fc35b`+`1958fa9` barra inferior; `8003bb3` redondeo 15px; `b8bccd1` notificaciones claras + acento; `20d282b` botones teal + badge; `de7d068` re-proteger. `tsc`/`build` pasan. Ritual: `ReportModal.tsx` y `ProfileVideoOverlay.tsx` se sacaron temporalmente de `.protected-files` y se re-agregaron. `Header.tsx` y `BottomNav.tsx` NO están en `.protected-files`.
- **Colaboraciones / Etiquetados con aprobación**: nueva tabla `video_tags` (migración `00015`, **aplicada**, idempotente con `DROP POLICY IF EXISTS`). Solo el subidor etiqueta creadores como colaboradores (subir y editar); cada etiquetado debe **aprobar/rechazar** desde `/notificaciones` (tipo `videoTag` con botones). Perfiles `/profile` y `/user/[id]` con pestañas **"Videos | Etiquetados"** (solo creadores) reutilizando `ProfileVideoCard`/`ProfileVideoOverlay`; el contador de videos no mezcla etiquetados. APIs `POST /api/video-tags/batch` (reemplazo total del set + notificación `videoTag` al etiquetado; valida rol `creator`, no deactivated/deleted, no auto-etiqueta) y `POST /api/video-tags/[id]/respond` (solo etiquetado, `approved`/`rejected`, notifica `videoTagAccepted`/`videoTagRejected` al subidor). Selector `src/components/CollaboratorInput.tsx` (búsqueda @username/display_name de creadores, chips, ruta `videoTags` en i18n × 9). Edición prefiltra colaboradores existentes. Subida captura el id del video para etiquetar tras el insert. `VideoTag`/`VideoTagStatus` en `src/types/index.ts`; hook `useTaggedVideos` en `useVideos.ts`. No se tocó ningún archivo de `.protected-files`. `npx tsc --noEmit` y `npm run build` pasan. Commit/push por orden del usuario.
